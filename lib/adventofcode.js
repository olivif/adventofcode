...
function isNiceStringV2(input) {
	...
	var foundTwoLetters = false;
	var foundRepeatingChar = false;
	for (var index = 0; index < input.length - 2; index++) {
		var twoLetters = input[index] + input[index+1];
		
		if (input.indexOf(twoLetters, index + 1) !== -1) {
	...
	if (input[input.length - 2] === input[input.length]) {
		foundRepeatingChar = true;
	}
		
	return foundTwoLetters && foundRepeatingChar;
}